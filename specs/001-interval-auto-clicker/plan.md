...
**Project Type**: 单模块 Android 应用（`:app`），package `com.gogoend.autotapper`
...
沿用现有单模块结构 `app/`，在 `com.gogoend.autotapper` 下按职责分包：

实际落地结构（单模块 `app/`，package `com.gogoend.autotapper`）：

```text
app/src/main/java/com/gogoend/autotapper/
...
app/src/test/java/com/gogoend/autotapper/      # JUnit4：ClickScheduler、CountdownModel、OverlayLayout
```
